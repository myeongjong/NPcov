...
p2            <- ggplot() +
  ...
  geom_line(data = data.frame(x = fit_us_swave$eval, y = fit_us_swave$yhat * vhat_swave), aes(x = x, y = y), color = "#E41A1C", lwd = 1.2, lty = "twodash", alpha = 0.75) +
  theme_bw() + xlab(NULL) + ylab(NULL) +
  geom_hline(yintercept = 0, linetype="solid", color = "black") +
  coord_cartesian(ylim = c(-0.8, 1.2) * vhat_swave)
